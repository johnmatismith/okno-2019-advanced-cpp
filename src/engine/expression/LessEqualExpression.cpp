...
LessEqualExpression::LessEqualExpression(std::unique_ptr<Expression>&& left, std::unique_ptr<Expression>&& right)
        : BinaryExpression("<=", std::move(left), std::move(right)) {
}

Value LessEqualExpression::evaluate(Expression::Record const& record) const {
...
void LessEqualExpression::visit(ExpressionVisitor& visitor) const {
    visitor.accept(*this);
}
