...
LessExpression::LessExpression(std::unique_ptr<Expression>&& left, std::unique_ptr<Expression>&& right)
        : BinaryExpression("<", std::move(left), std::move(right)) {
}

Value LessExpression::evaluate(Expression::Record const& record) const {
